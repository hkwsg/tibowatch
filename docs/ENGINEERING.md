...
Only a matching `https://savemetibo.com/events/<event_id>/artifacts/<approval_id>.png` can supply `icon`. The optional `TIBOWATCH_ICON_URL` setting overrides the upstream icon for new selections. It accepts a publication PNG or this repository’s `assets/notification-icon.png` at a full 40-character commit URL on `raw.githubusercontent.com`; other hosts/paths are ignored. Like upstream icons, the chosen URL is persisted once, so changing the setting does not alter pending retries. No runtime image download is added. An optional icon that exceeds the 3000-byte request budget is removed before selection is saved; text is never truncated. Icon availability/cropping is best-effort, not a full-card attachment.
