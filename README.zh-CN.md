...
本文对应基于 RSS 的 v1.1 实现。部署提示词会先核对所选代码版本，再执行安装。
...
```text
请在我授权的 Linux 服务器上部署 https://github.com/hkwsg/tibowatch。
...
这里只展示内容示例。实际通知就是上游标题和正文，或它们的译文；不附加来源标签、时间或解释。点击通知打开 RSS 条目提供的链接。
...
第一次运行只建立历史基线，不补发旧消息。新增条目或正文改动才形成通知；重复轮询、仅日期或链接变化都不重复打扰。Bark 发送失败就保留待发内容，下次重试不会再次翻译。
...
不需要。平时由 systemd 调度脚本；开启翻译后，只有新消息才临时调用一次 Codex CLI，单次最多等待 30 秒。
